...
REM ===== DETECT ALL INSTALLATIONS (with status, all versions) =====
...
set "found=0"
REM Stable
for /f "delims=" %%D in ('dir /b /ad "%LOCALAPPDATA%\Discord\app-*" 2^>nul') do (
    if exist "%LOCALAPPDATA%\Discord\%%D\resources\app.asar" (
        set "choices=!choices!!idx!) Discord (Stable) [%%D] [Installed] (!LOCALAPPDATA!\Discord\%%D\resources)\n"
        set "path[!idx!]=!LOCALAPPDATA!\Discord\%%D\resources"
        set /a idx+=1
        set /a found+=1
    )
)
if !found! equ 0 (
    set "choices=!choices!!idx!) Discord (Stable) [Not installed]\n"
    set "path[!idx!]=NOT_INSTALLED"
    set /a idx+=1
)
set "found=0"
REM PTB
for /f "delims=" %%D in ('dir /b /ad "%LOCALAPPDATA%\DiscordPTB\app-*" 2^>nul') do (
    if exist "%LOCALAPPDATA%\DiscordPTB\%%D\resources\app.asar" (
        set "choices=!choices!!idx!) Discord PTB [%%D] [Installed] (!LOCALAPPDATA!\DiscordPTB\%%D\resources)\n"
        set "path[!idx!]=!LOCALAPPDATA!\DiscordPTB\%%D\resources"
        set /a idx+=1
        set /a found+=1
    )
)
if !found! equ 0 (
    set "choices=!choices!!idx!) Discord PTB [Not installed]\n"
    set "path[!idx!]=NOT_INSTALLED"
    set /a idx+=1
)
set "found=0"
REM Canary
for /f "delims=" %%D in ('dir /b /ad "%LOCALAPPDATA%\DiscordCanary\app-*" 2^>nul') do (
    if exist "%LOCALAPPDATA%\DiscordCanary\%%D\resources\app.asar" (
        set "choices=!choices!!idx!) Discord Canary [%%D] [Installed] (!LOCALAPPDATA!\DiscordCanary\%%D\resources)\n"
        set "path[!idx!]=!LOCALAPPDATA!\DiscordCanary\%%D\resources"
        set /a idx+=1
        set /a found+=1
    )
)
if !found! equ 0 (
    set "choices=!choices!!idx!) Discord Canary [Not installed]\n"
    set "path[!idx!]=NOT_INSTALLED"
    set /a idx+=1
)
set "choices=!choices!!idx!) Enter custom location manually\n"
(for /f "delims=" %%l in ("!choices!") do @echo %%l)
endlocal & goto :eof

REM ===== SELECT INSTALLATION (all options, all versions) =====
:select_discord_installation
setlocal EnableDelayedExpansion
call :detect_discord_installations_full
set /a maxidx=0
for /f "tokens=1 delims=)" %%I in ('call :detect_discord_installations_full ^| findstr /r "^[0-9]\+)"') do set /a maxidx=%%I
:choose_discord_full
set /p sel="Select an option [1-%maxidx%]: "
if "%sel%"=="%maxidx%" goto custom_location
set "chosen_path=!path[%sel%]!"
if "%chosen_path%"=="NOT_INSTALLED" (
    echo [WARN] This installation is not present. Use a different option or enter a custom path.
    pause
    goto choose_discord_full
)
if not exist "!chosen_path!\app.asar" (
    set /p confirm="[WARN] app.asar not found in this path. Use anyway? (y/N): "
    if /I "!confirm!"=="y" set "DISCORD_RESOURCES=!chosen_path!" & endlocal & goto main_menu
    goto choose_discord_full
) else (
...
:custom_location
set /p user_path="Enter the full path to the folder containing app.asar: "
if exist "%user_path%\app.asar" (
    set "DISCORD_RESOURCES=%user_path%"
    goto main_menu
) else (
    echo [ERROR] app.asar not found in %user_path%. Try again.
    pause
    goto custom_location
)
...
if "%DISCORD_RESOURCES%"=="" call :select_discord_installation
call :backup_asar
goto main_menu 